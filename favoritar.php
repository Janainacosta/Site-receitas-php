...
<?php session_start();
  ...
  $sql_insert = "INSERT INTO favoritar (idcadastro_usuario, idcadastro_receita) VALUES ('$usuario','$cod')";

  print_r($sql_insert);

  if(mysqli_query($con,$sql_insert)){
    echo "<script>alert('Obrigada por favoritar');window.location='index.php.#section2'</script>";
  }else{
    echo "<script>alert('Você precisa ter um Cadastro para Favoritar a receita: se já possui, efetue Login, caso não possua, faça seu cadastro');window.location='index.php.#section2'</script>";
  }
